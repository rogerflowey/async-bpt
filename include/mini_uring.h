...
// --- Syscall Wrappers ---
// We need to wrap the raw syscalls as glibc might not have wrappers for them.
...
// We need to define our own io_uring struct to hold all the ring pointers and metadata.
// This mimics the structure used internally by liburing.
struct io_uring {
    struct io_uring_sq {
        unsigned *head;
        unsigned *tail;
    ...
    // Application-side cache of the SQ tail.
...
static inline int io_uring_queue_init(unsigned int entries, struct io_uring *ring, unsigned int flags) {
    ...
    // Memory map the submission and completion rings
    ...
    // Map the submission queue entries (SQEs)
    ...
    // Populate our custom io_uring struct with pointers to the mapped regions
...
static inline void io_uring_queue_exit(struct io_uring *ring) {
    size_t sq_ring_sz = *ring->sq.ring_entries * sizeof(unsigned) + sizeof(struct io_uring_sqe);
    size_t cq_ring_sz = *ring->cq.ring_entries * sizeof(struct io_uring_cqe) * 2; // Approximation

    // This is a simplified unmap. A more robust version would store the exact sizes from init.
    munmap(ring->sqes, *ring->sq.ring_entries * sizeof(struct io_uring_sqe));
    munmap(ring->sq.head, sq_ring_sz); // Unmap the whole region via one of its pointers
    munmap(ring->cq.head, cq_ring_sz); // Unmap the whole region via one of its pointers
...
static inline int io_uring_submit(struct io_uring *ring) {
    unsigned submitted = ring->sq_tail_cached - *ring->sq.tail;
    if (submitted == 0) {
        return 0;
    }

    // Make the new tail visible to the kernel
...
// This macro perfectly replicates the behavior of liburing's for-each loop.
